...
export function LoginPage() {
  const { signIn, signUp, signInWithGoogle, passwordRecoveryMode, sendPasswordResetEmail, updatePassword } = useAuth();
  const { showToast } = useToast();

  type Screen = 'login' | 'signup' | 'forgot' | 'forgot-sent' | 'reset';
  const [screen, setScreen] = useState<Screen>(passwordRecoveryMode ? 'reset' : 'login');
  ...
  // Sync when passwordRecoveryMode flips true (e.g. on page load after redirect)
  if (passwordRecoveryMode && screen !== 'reset') {
    setScreen('reset');
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: '#1F1D1C', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          <Logo />
          <div style={{ fontFamily: '"Century Gothic","Trebuchet MS",sans-serif', fontSize: '11px', fontWeight: 700, color: '#FABE3D', letterSpacing: '.1em', textTransform: 'uppercase' }}>
        ...
        {screen === 'forgot-sent' && (
          <div style={{ background: 'rgba(250,190,61,.1)', border: '1px solid rgba(250,190,61,.3)', borderRadius: '10px', padding: '24px 20px', textAlign: 'center' }}>
            <div style={{ fontSize: '13px', fontWeight: 700, color: '#FABE3D', marginBottom: '8px' }}>Check your email</div>
            <div style={{ fontSize: '12px', color: 'rgba(255,255,255,.6)', lineHeight: 1.7 }}>
              A password reset link has been sent to{' '}
              <strong style={{ color: 'rgba(255,255,255,.85)' }}>{email}</strong>.<br />
              Click the link in that email to set a new password.
            </div>
            <button type="button" onClick={() => setScreen('login')} style={{ ...ghostBtn, marginTop: '14px', textDecoration: 'underline', display: 'block', margin: '14px auto 0' }}>
              Back to sign in
            </button>
          </div>
        )}
        ...
        {screen === 'reset' && (
          <form onSubmit={handleSetNewPassword} style={{ background: 'rgba(255,255,255,.05)', borderRadius: '14px', padding: '28px', border: '1px solid rgba(255,255,255,.1)' }}>
            ...
            <button type="submit" disabled={loading} style={{ ...primaryBtn, opacity: loading ? 0.7 : 1, cursor: loading ? 'not-allowed' : 'pointer' }}>
        ...
        {screen === 'forgot' && (
          <form onSubmit={handleForgotPassword} style={{ background: 'rgba(255,255,255,.05)', borderRadius: '14px', padding: '28px', border: '1px solid rgba(255,255,255,.1)' }}>
            <div style={{ marginBottom: '22px' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: '16px' }}>
              <button type="button" onClick={() => setScreen('login')} style={ghostBtn}>
                Back to sign in
              </button>
            </div>
          </form>
        ...
        {/* ── AWAITING EMAIL CONFIRMATION (signup) ── */}
        {awaitingConfirmation && (screen === 'login' || screen === 'signup') && (
          <div style={{ background: 'rgba(250,190,61,.1)', border: '1px solid rgba(250,190,61,.3)', borderRadius: '10px', padding: '16px 18px', marginBottom: '20px', textAlign: 'center' }}>
            ...
            <button type="button" onClick={() => { setAwaitingConfirmation(false); setScreen('login'); }} style={{ marginTop: '10px', fontSize: '11px', color: '#FABE3D', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
              Back to sign in
            </button>
          </div>
        )}
        ...
        {/* ── LOGIN / SIGNUP ── */}
        {(screen === 'login' || screen === 'signup') && !awaitingConfirmation && (
          <>
            ...
            <form onSubmit={screen === 'login' ? handleSignIn : handleSignUp} style={{ background: 'rgba(255,255,255,.05)', borderRadius: '14px', padding: '28px', border: '1px solid rgba(255,255,255,.1)' }}>
              {screen === 'signup' && (
                <div style={{ marginBottom: '14px' }}>
              ...
              <div style={{ marginBottom: screen === 'login' ? '10px' : '22px' }}>
              ...
              {screen === 'login' && (
                <div style={{ textAlign: 'right', marginBottom: '18px' }}>
                  <button
                    ...
                    style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,.4)', fontSize: '11px', cursor: 'pointer', fontFamily: 'Verdana,sans-serif' }}
              ...
              <div style={{ textAlign: 'center', marginTop: '16px' }}>
                <button
                  type="button"
                  onClick={() => setScreen(s => s === 'login' ? 'signup' : 'login')}
                  style={ghostBtn}
                >
                  {screen === 'login' ? "Don't have an account? Sign up" : 'Already have an account? Sign in'}
                </button>
              </div>
            </form>
          </>
